 ...
 * - Removed YIN pitch detection 
 * - Now 4 modes: LFO, WAVE, SPECTRUM, TUNER
 * - Reduced code size and RAM usage
 * - Timing functions have ben refined so SHOT mode is obsolete.
 * - Should remember and restore last mode + settings at boot
 * - Change to HW SPI!! This does not work with SCOPEv1 and SCOPEv2 HW. Its a WIP
  */
...
// New pin definitions after rewiring!!
#define OLED_DC    9
#define OLED_RESET 8
#define OLED_MOSI 11
#define OLED_CS 10
...
// Wave mode state (moved from static to allow reset on mode change)
uint8_t waveState = 0;
...
void setup() {
  ...
  loadAllSettings();
  setupMode(mode);
  old_mode = mode;  // Initialize old_mode to prevent false mode-change on first loop
}
...
void loop() {
  ...
  // Determine max param based on mode (Wave mode has 4 menu items)
  ...
  // Wrap param (menu selection) - max depends on mode
  maxParam = (mode == MODE_WAVE) ? 4 : 3;
  ...
  // Wrap param1, param2, param3 based on current mode
  switch (mode) {
    case MODE_LFO:
      // param1: 1-8, param2: -6 to 10
      ...
      break;
    case MODE_WAVE:
      // param1: 1-9 (9=AUTO), param2: -6 to 10 (offset), param3: 1-6 (Rfrs)
...
void loadAllSettings() {
  for (uint8_t m = 0; m < NUM_MODES; m++) {
    int baseAddr = EEPROM_PARAM_SELECT_ADDR + (m * 4);  // 4 bytes per mode
    ...
    modeSettings[m].param3 = EEPROM.read(baseAddr + 3);
    
    // Mode-specific param_select validation (Wave has 4 params, others have 3)
    uint8_t maxParamSelect = (m + 1 == MODE_WAVE) ? 4 : 3;
    if (modeSettings[m].param_select > maxParamSelect) modeSettings[m].param_select = 0;
    
    // Initialize param3 to safe default for all modes
    if (m + 1 != MODE_WAVE) {
      modeSettings[m].param3 = 1;  // Not used, but initialize to safe value
    }
    ...
    switch (m + 1) {
      case MODE_LFO:
        ...
        break;
      case MODE_WAVE:
        // param1: Time/AUTO, param2: Offset, param3: Rfrs
        modeSettings[m].param1 = constrain(modeSettings[m].param1, 1, 9);  // 9 = AUTO
        modeSettings[m].param2 = constrain(modeSettings[m].param2, -6, 10);  // Offset
        modeSettings[m].param3 = constrain(modeSettings[m].param3, 1, 6);   // Rfrs
        if (modeSettings[m].param1 == 0) modeSettings[m].param1 = 9;  // Default to AUTO
        if (modeSettings[m].param2 == 0) modeSettings[m].param2 = 1;
...
void runWaveMode(bool showParams) {
  ...
  switch (waveState) {
    ...
    case 2:  // Process and display
      {
        // Find min/max for scaling and trigger
        uint8_t minVal = 255, maxVal = 0;
        ...
        if (range > 20) {
          ...
          for (uint8_t i = 1; i < 64; i++) {  // Search first half for trigger
        ...
        // Auto-frequency detection and timebase adjustment
        if (autoMode && range > 20) {
          // Count zero crossings for frequency estimate
          uint8_t crossings = 0;
          uint16_t firstCross = 0, lastCross = 0;
          uint8_t hysteresis = range / 6;
          if (hysteresis < 5) hysteresis = 5;
          uint8_t threshHigh = center + hysteresis;
          uint8_t threshLow = center - hysteresis;
...
void drawParameterBar(bool showParams) {
  ...
  if (param == 1) display.drawFastHLine(0, 8, 28, WHITE);
  ...
  if (isWaveMode) {
    // Wave mode: compact layout for 4 items
  ...
  if (isWaveMode) {
    ...
    display.print(F("Rfrs:"));
    display.print(param3);
    if (param == 4) display.drawFastHLine(90, 8, 38, WHITE);
    ...
    if (param == 3) display.drawFastHLine(84, 8, 44, WHITE);
  }
}
